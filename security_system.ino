 ...
 * Token-Based Login with Full Dashboard
...
String getLoginPage() {
    ...
    html += ".then(r=>r.json()).then(d=>{if(d.token){localStorage.setItem('authToken',d.token);m.className='msg success';m.textContent='Login ok!';m.style.display='block';";
    html += "setTimeout(()=>location.href='/dashboard',500)}else{m.className='msg error';m.textContent=d.error||'Login failed';m.style.display='block'}})};";
    html += "</script></body></html>";
    return html;
}
...
String getDashboardPage() {
    ...
    html += "<div class=header><h1>Dashboard</h1></div>";
    ...
    html += "<div class=footer>(C) 2025 Community-Based Industrial IoT</div>";
    html += "</div>";
...
void setupRoutes() {
    ...
    server.on("/dashboard", HTTP_GET, []() {
        // Check if session is still valid (no token parameter needed, check from last login)
    ...
    server.on("/api/signup", HTTP_POST, []() {
        ...
        if (userData.isSignupDone) {
            server.send(400, "application/json", "{\"error\":\"Account exists\"}");
            return;
        }
        if (!isValidInput(user, 3, 20)) {
            server.send(400, "application/json", "{\"error\":\"Invalid username\"}");
            return;
        }
        if (!isStrongPassword(pass)) {
            server.send(400, "application/json", "{\"error\":\"Weak password\"}");
            return;
        }
...
void setup() {
    ...
    Serial.println("SSID: SecuritySystem");
    ...
    Serial.println("http://192.168.4.1/signup");
    Serial.println("=====================\n");
}
