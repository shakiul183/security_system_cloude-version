...
// --- GLOBALS ---
UserData userData;                      //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
...
/////////////////////////////////////////////////////////////////////
SemaphoreHandle_t userDataMutex;
bool sensorsEnabled = true;
bool sensorDisable_Beep = true;
...
String callerNumber = "";
String smsContent = "";
int convertedSMScontent = 0;

// -----------------------
// DUMMY PARSER FUNCTIONS
// (You must replace with your actual logic)
// -----------------------
String parseCallerNumber(String msg) {
...
String parseSenderNumber(String msg) {
    int start = msg.indexOf("\"");
    int end   = msg.indexOf("\"", start + 1);
    if (start != -1 && end != -1) {
        return msg.substring(start + 1, end);
...
int prev1 = HIGH, prev2 = HIGH, prev3 = HIGH, prev4 = HIGH, prev5 = HIGH;
...
void webServerTask(void *parameter) {
  ...
  while (1) {
    ...
    /////////////////////////////////////////////////////////////
        ...
        // -------- Parse Incoming Call (+CLIP)
        if (receivedMessage.indexOf("+CLIP:") != -1) {
            callerNumber = parseCallerNumber(receivedMessage);
            Serial.println("Caller: " + callerNumber);
        }

        // -------- Parse Incoming SMS (+CMT)
        if (receivedMessage.indexOf("+CMT:") != -1) {

            smsContent = Serial2.readStringUntil('\n'); // Read SMS body
            convertedSMScontent = smsContent.toInt();   // Convert number
            String senderNumber = parseSenderNumber(receivedMessage);

            Serial.println("SMS From: " + senderNumber);
            Serial.println("SMS Body: " + smsContent);
        }
    }///////////////////////////////////////////
...
void core1Task(void *parameter) {
  ...
  while (1) {
       ...
       // readToggleMode();
/*
        if (userData.toggleMode == 1) {
    // Full Features Mode
                    Serial.println("fullFeatures MOde");
                 } 
        else {
            // Beep Only Mode
            Serial.println("Beep Mode");
            
                }

*/

if (xSemaphoreTake(userDataMutex, portMAX_DELAY) == pdTRUE) {
  if (userData.toggleMode == 1) {
    Serial.println("Beep Mode");
     sensorsEnabled = false;
     sensorDisable_Beep = true;
  } else {
    Serial.println("Full Features");
    sensorsEnabled = true;
    sensorDisable_Beep = false;
  }
  xSemaphoreGive(userDataMutex);
}
...
void setup() {
 ...
 // Create mutex
  userDataMutex = xSemaphoreCreateMutex();
  ...
  randomSeed(analogRead(36) + millis());
      ...
      Serial2.println("AT+DDET=1");  // DTMF
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    Serial2.println("AT+CMGF=1"); // Set SMS text mode
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    Serial2.println("AT+CSCS=\"GSM\""); // Set character set
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    Serial2.println("AT+CNMI=2,2,0,0,0"); // Enable immediate SMS notifications
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    Serial2.println("AT+CPMS=\"SM\""); // Use SIM storage for SMS
    vTaskDelay(1000 / portTICK_PERIOD_MS);
...
void buzzer_High_Once() {
   digitalWrite(BuzzerPin, HIGH);   // Beep ON
    vTaskDelay(pdMS_TO_TICKS(50)); // 1 second non-blocking delay
...
void checkAllSensors() {
  if (!sensorsEnabled) return;  // 👈 skip if disabled
  int s1 = digitalRead(INPUT_PIN_1);
  int s2 = digitalRead(INPUT_PIN_2);
  int s3 = digitalRead(INPUT_PIN_3);
  int s4 = digitalRead(INPUT_PIN_4);
  int s5 = digitalRead(INPUT_PIN_5);

  if (s1 != prev1) { prev1 = s1; if (s1 == LOW) triggerAction(1); digitalWrite(BuzzerPin, HIGH); }
  if (s2 != prev2) { prev2 = s2; if (s2 == LOW) triggerAction(2); }
  if (s3 != prev3) { prev3 = s3; if (s3 == LOW) triggerAction(3); }
  if (s4 != prev4) { prev4 = s4; if (s4 == LOW) triggerAction(4); }
  if (s5 != prev5) { prev5 = s5; if (s5 == LOW) triggerAction(5); }
}

// ================================
// ✅ Action when sensor goes LOW
// ================================
void triggerAction(uint8_t sensorNum) {
...
void checkAllSensors_For_beep() {
  if (!sensorDisable_Beep) return;  // 👈 skip if disabled
  int s1 = digitalRead(INPUT_PIN_1);
  int s2 = digitalRead(INPUT_PIN_2);
  int s3 = digitalRead(INPUT_PIN_3);
  int s4 = digitalRead(INPUT_PIN_4);
  ...
  if (s1 != prev1) { prev1 = s1; if (s1 == LOW) triggerAction2(1); buzzer_High_Once();}
  if (s2 != prev2) { prev2 = s2; if (s2 == LOW) triggerAction2(2); buzzer_High_Once();}
  if (s3 != prev3) { prev3 = s3; if (s3 == LOW) triggerAction2(3);buzzer_High_Once(); }
  if (s4 != prev4) { prev4 = s4; if (s4 == LOW) triggerAction2(4); buzzer_High_Once();}
  if (s5 != prev5) { prev5 = s5; if (s5 == LOW) triggerAction2(5); buzzer_High_Once();}
}

// ================================
// ✅ Action when sensor goes LOW
// ================================
void triggerAction2(uint8_t sensorNum) {
  Serial.print("[TRIGGER] Sensor ");
  Serial.print(sensorNum);
  Serial.println(" detected LOW!");
}
